...
4. **Update the canvas.** Maintain the "Slack Todos" Slack canvas (load canvas tools via ToolSearch if needed):
   ...
   - **Don't short-circuit on "nothing new".** A user-checked box or a due daily-drain is itself a reason to rewrite the canvas. Even when no new messages arrived, still `slack_read_canvas` and, if any active item is checked `[x]` or `last_drain` is before today, run `slack_update_canvas` to apply the move/drain. Only skip the write when the canvas is already fully correct (no new messages, nothing checked, no drain due).
